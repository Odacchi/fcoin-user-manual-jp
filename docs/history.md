...
### 2020/6/23 債権ロック解除計画のお知らせ

FUSDをBTCとして徐々に返すプランが発表されました。  
要約すると以下のような内容です。

* 本計画はFCoinへの債権を持つユーザー（=FUSDを保有するユーザー）が対象であり、債権のないユーザーには関係ありません。
* FUSD（=ドル建ての債権）はFMexプラットフォームでの「取引への貢献度」に応じて徐々にBTCに変換されるカタチで返金されます。
* 「取引への貢献度」は、これまでの取引マイニング、未約定注文マイニング、直近板並べマイニングと同じような仕組みによって計算されます。それぞれ取引ロック解除、保留注文ロック解除、直近板並べロック解除という名称です。全ての仕組みは独立しており、条件を満たせばすべてのロック解除に参加することができます。
* 取引ロック解除は、取引した人に取引量に応じてFUSDをBTCに引き換えしていく仕組み※です。
* 保留注文ロック解除は、約定を問わず指値している人のFUSDをBTCに引き換えしていく仕組み※です。
* 直近板並べロック解除は、約定を問わず指値している人のFUSDをBTCに引き換えしていく仕組み※です。保留注文ロック解除との大きな違いは、板の上下15段の範囲内に指値している人だけに対象を絞っている点です。
* 直近板並べ、取引ロック解除、保留注文ロック解除は1日の総返金額を5:3:2の比率で返金します（今後調整される可能性があります）。
* 極端に取引量が多いユーザーには返金しすぎないような計算式が適用されます。
* 1日の総変金額の計算方法は本発表には含まれません。あくまで総変金額をどのように分配するかの説明のみです。

※詳細は [Telegram](https://t.me/fcoinfanjapanese/1508) を参照してください。

原文は[コチラ（中国語）](https://fnotice.zendesk.com/hc/zh-cn/articles/360049970774-%E5%80%BA%E5%8A%A1%E8%A7%A3%E9%94%81%E6%96%B9%E6%A1%88%E5%85%AC%E5%91%8A)
