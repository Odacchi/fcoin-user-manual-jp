...
|:-----------------|:-----------------------|
| 参加方法          | 在指定交易对成交 |
| 参加条件          | 全员都可以参加，但是完成了KYC认证的用户效率更高 |
| FT的发放时间 | 交易成交次日（GMT+8） |

    
### 解说
在指定交易对成交，根据对交易额的贡献，次日会发放挖矿报酬部分的FT。  
交易对不同，当日的挖矿额度不同，定期会调整挖矿额度。  
根据用户所贡献的交易额占各个交易对的交易额的比重，计算最终所发放的FT挖矿奖励的数量。  
挖矿所能获得的FT奖励，大抵可以通过下列公式进行计算*1。

```
次日发放的FT ≒ 该交易对每日挖矿额度 ✕ (用户在该交易对贡献的交易量 / 该交易对交易总量)
```
*1 KYC认证完毕，且当日下单笔数小于等于50笔（取消的笔数也计入统计）时（后面会详细介绍）

虽然全员都可以参与交易挖矿，但是如果完成了KYC，并且一天之内的下单笔数在50次以内的话，挖矿的效率是最大的。  
相反，如果没有KYC认证，并且一天内下单笔数超过50次（包含取消的订单）的话，基本上没什么效率。  
